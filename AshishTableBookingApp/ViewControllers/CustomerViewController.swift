...
class CustomerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.customerAssignedTable(notification:)), name: Notification.Name(ASFirebaseDataSource.CustomerAssignedTable), object: nil)
    }
    ...
    @objc func customerAssignedTable(notification: Notification){
        if let notificationDict = notification.userInfo as? [String:String] {
            if let phoneNo = notificationDict[Utilities.phonenumber] {
